...
| [`@fablabfortsmith/purelymail-config`](./packages/config) | Shared profile/config loader (TOML → registry; env-var / OS-keychain tokens; `[notify]` SMTP). Used by the CLI and TUI.             | (transitive)                               |
| [`@fablabfortsmith/purelymail-notify`](./packages/notify) | SMTP mailer + new-account welcome-email template (nodemailer). Powers `--notify`.                                                   | (transitive)                               |
